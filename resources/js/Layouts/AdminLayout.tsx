...
const AdminLayout: React.FC<AdminLayoutProps> = ({ children, title = 'Admin' }) => {
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#f7f7f7' }}>
      <nav className="border-b" style={{ backgroundColor: '#B71C1C', borderBottomColor: '#FFD700' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-6">
            <Link href="/" className="text-lg font-semibold" style={{ color: '#FFD700' }}>TowerTrack</Link>
          ...
          <div className="flex items-center gap-4">
            <span className="text-sm" style={{ color: '#FFD700' }}>{user?.name} ({user?.role})</span>
