...
export default function LocationCard({
  ...
  const label = isFoPoint ? 'Lokasi Fiber Optik' : 'Lokasi Menara';

  // Icon for Fiber Optik (different from Tower)
  // Icon for Fiber Optik (different from Tower)
  const locationIcon = isFoPoint ? (
    <svg className="w-5 h-5 text-destructive-strong" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div className="bg-card rounded-lg p-5 border border-border shadow-sm">
      <div className="flex items-center gap-2 mb-3">
        <div className="rounded-lg border border-primary-border bg-primary-soft p-2 text-primary-strong">
          {locationIcon}
        </div>
