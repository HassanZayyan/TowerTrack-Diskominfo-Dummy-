...
export default function MessageCard({ item, getStatusColor, formatDate, onOpen, hideEmail = false }: MessageCardProps) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 hover:shadow-lg transition-all duration-200 group">
      {/* Header: User & Status */}
      <div className="flex justify-between items-start mb-3">
        <div className="flex items-center gap-3">
          {/* Avatar */}
          <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold ${item.type === 'Keluhan' ? 'bg-red-50 text-red-600' : 'bg-teal-50 text-teal-600'
            }`}>
            {item.senderName.charAt(0).toUpperCase()}
          </div>
          <div>
            <h3 className="font-semibold text-gray-900 leading-tight">{item.senderName}</h3>
            <p className="text-xs text-gray-500 mt-0.5">{formatDateWithTime(item.created_at)}</p>
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium ${item.type === 'Keluhan' ? 'bg-red-50 text-red-700' : 'bg-teal-50 text-teal-700'
          }`}>
          {item.type}
        </span>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-gray-50">
        ...
        <button
          ...
          className="text-xs font-semibold text-gray-900 hover:text-red-600 flex items-center transition-colors"
        >
          Lihat Detail
