...
/**
 * The wizard has three states and they are now stated on the page instead of
 * being inferred from which buttons happen to be enabled.
 */
const STEPS = [
...
/* ---------------------------------------------------------------------------
   Local recipes. These existed as copy-pasted class strings at ~8 sites each;
   naming them once is what keeps the three import screens on one spec.
   ------------------------------------------------------------------------- */
...
const TowerImportPage: React.FC<Props> = ({ templateUrl, availableOwners }) => {
  ...
  return (
    <AdminLayout title="Impor Menara">
      <Head title="Impor Menara" />

      <PageHeader
        title="Impor Data Menara"
        description="Impor data menara dari templat Excel (.xlsx atau .xls)"
        showLogo={false}
        actions={
          <Button asChild variant="outline" className="h-11">
            <Link href={route('admin.towers.index')}>
              <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
              </svg>
              Kembali ke Daftar Menara
            </Link>
          </Button>
        }
      />
      ...
      <div className="space-y-5">
              ...
              <CardContent className="space-y-5">
                ...
                {availableOwners.length > 0 && (
                  <div className="space-y-1.5 border-t border-border/70 pt-5">
                    <label htmlFor="import-owner" className={FIELD_LABEL}>
                      Owner <span className="font-normal text-muted-foreground">(Opsional)</span>
                    </label>
                    <select
                      id="import-owner"
                      value={ownerId}
        ...
        {previewData && (
          <div className="space-y-5">
            {/* Missing required columns — blocks the import, so it leads. */}
            ...
            {/* Row-level errors. Every failing row is listed with its reason —
                the old version cut the list at 10 and said "dan N lainnya". */}
            {errorRowCount > 0 && (
            ...
            {/* Column mapping: which DB field came from which Excel header. */}
            <Card padding="none">
              <CardHeader>
